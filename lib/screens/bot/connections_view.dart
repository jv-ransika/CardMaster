...
class _ConnectionsViewState extends State<ConnectionsView> {
  ...
  Future<void> _connectAllDevices() async {
    ...
    for (var key in widget.controller.deviceAddresses.keys) {
      String? address = widget.controller.deviceAddresses[key];
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          SizedBox(height: 16),
          ...widget.controller.deviceAddresses.keys.map((key) {
            return Container(
              ...
              child: Row(
                children: [
                  Container(
                    width: 16,
                    height: 16,
                    decoration: BoxDecoration(color: _getStatusColor(widget.controller.deviceAddresses[key], deviceConnections[key]), shape: BoxShape.circle),
                  ),
                  SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(widget.controller.deviceAddresses[key] ?? "Not Paired", style: TextStyle(fontSize: 14, color: Colors.grey[600])),
                      ],
                    ),
                  ),
                ],
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  title: Text("Clear All Addresses"),
                  content: Text("Are you sure you want to remove all saved device addresses?"),
                  actions: [
